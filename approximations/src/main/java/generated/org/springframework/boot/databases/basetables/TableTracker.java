...
public class TableTracker {
    ...
    public static <T> void tryTrack(String tableName, T value, int entityIx, Class<T> type) {
        ...
        if (ix == -1) {
            ...
            if (entityIx != 0) SpringEngine._internalLog("[DB Warning] Unexpected index in tryTrack");
        ...
        if (entityIx != oldEntityIX + 1) SpringEngine._internalLog("[DB Warning] Unexpected entity index in tryTrack");
