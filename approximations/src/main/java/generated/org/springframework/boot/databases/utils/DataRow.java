...
public class DataRow {
    ...
    private int indexOf(String entityName, boolean withWarning) {
        ...
        if (withWarning) SpringEngine._internalLog("[DB Warning] In DataRow on entity with name ", entityName);
        return -1;
    }
    ...
    public DataRow merge(DataRow other) {
        ...
        for (int i = 0; i < otherEntitiesNames.size(); i++) {
            ...
            if (indexOf(entityName, false) != -1) {
                SpringEngine._internalLog("[DB Warning] Both DataRows for merge contain entity with name: ", entityName);
            }
